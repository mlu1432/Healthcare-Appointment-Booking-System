// /app/(route)/profile/medical/components/AlertMessage.jsx
...
export const AlertMessage = ({ type, message, onClose }) => {
    ...
    return (
        <div className={`mb-4 p-4 ${bg} border ${border} rounded-lg`}>
            <div className="flex">
                <div className="flex-shrink-0">
                    <svg className={`h-5 w-5 ${text}`} viewBox="0 0 20 20" fill="currentColor">
                ...
                {onClose && (
                    <button
                        type="button"
                        onClick={onClose}
                        className="ml-auto pl-3 -mr-1.5 -mt-0.5"
                    >
